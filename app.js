...
let currentFilter = 'all'; // ★ 'all', 'home', 'search'
...
// ★ 以前の棚（'daily-report-app'）のデータを見たい場合は、ここを 'daily-report-app' に変更して 'git push' してください。
...
const filterButtonContainer = document.getElementById('affiliation-filter-buttons');
...
function setupEventListeners() {
    ...
    // ★フィルターボタンのリスナー
    filterButtonContainer.addEventListener('click', handleFilterClick);
...
function renderDailyUserList() {
    ...
    const filteredUsers = allUsers.filter(user => {
        const affiliation = user.affiliation || 'none';
        if (currentFilter === 'home') {
            return affiliation === 'home' || affiliation === 'both';
        }
        if (currentFilter === 'search') {
            return affiliation === 'search' || affiliation === 'both';
        }
        return true; // 'all'
    });
    ...
    // ★フィルターされたユーザー (filteredUsers) を元にリストを作成
    ...
    const renderUserCard = (user, attendance, reportSent, recordId) => {
        ...
        // ★修正: reportSentTextを動的に変更
        const reportSentText = (isPresent && reportSent) ? '✓ 報告済' : ''; // チェックありの時だけ表示
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                    ...
                    <label class="report-sent-checkbox-label flex items-center mb-3 sm:mb-0">
                        ...
                        <div class="flex flex-col ml-3">
                            <!-- ★修正: 氏名の隣に呼び方を表示 -->
                            <span class="font-bold text-lg">${user.name} <span class="text-sm text-gray-500 font-normal ml-2">(${user.nickname})</span></span>
                            <!-- ★修正: チェックが入っている時だけ「✓ 報告済」を表示 -->
                            <span class="report-sent-label-text ${reportSent ? 'text-green-600 font-medium' : 'text-gray-400'}">${reportSentText}</span>
                        </div>
                    </label>
    ...
    if (homeUsers.length > 0) {
        // ★修正: 見出しに人数を追加
        finalHtml += `<div class="mb-4"><h3 class="text-lg font-semibold text-gray-700 mb-2 border-b pb-1">ホーム出席者 (${homeUsers.length}名)</h3><div class="space-y-3">`;
    ...
    if (searchUsers.length > 0) {
        // ★修正: 見出しに人数を追加
        finalHtml += `<div class="mb-4"><h3 class="text-lg font-semibold text-gray-700 mb-2 border-b pb-1">サーチ出席者 (${searchUsers.length}名)</h3><div class="space-y-3">`;
    ...
    if (filteredUsers.length === 0) {
         finalHtml = '<p class="text-center text-gray-500 py-8">該当する所属の利用者がいません。</p>';
    }
...
function renderUserManagementList() {
    userManagementList.innerHTML = '';
     ...
     const affiliationMap = {
        'home': 'ホーム',
        'search': 'サーチ',
        'both': 'ホーム＆サーチ', // ★「両方」を「ホーム＆サーチ」に変更
        'none': '未設定'
    };

    allUsers.forEach(user => {
        const affiliationText = affiliationMap[user.affiliation || 'none'];
        const card = `
            <div class="bg-white p-4 rounded-lg shadow-sm flex flex-col sm:flex-row justify-between sm:items-center">
                <div class="mb-2 sm:mb-0">
                    ...
                    <!-- ★所属事業所を表示 -->
                    <p class="text-sm text-gray-600">所属: <span class="font-medium text-gray-800">${affiliationText}</span></p>
                </div>
                <!-- スマホではボタンを右寄せ -->
...
async function handleUserFormSubmit(e) {
    ...
    const user = {
        ...
        affiliation: document.getElementById('user-affiliation').value, // ★所属を保存
    };
    try {
...
function handleFilterClick(e) {
    ...
    clickedButton.classList.remove('filter-btn-inactive');
...
function openUserModalForCreate() {
    ...
    document.getElementById('user-affiliation').value = 'none'; // ★デフォルト所属
...
function openUserModalForEdit(user) {
    ...
    document.getElementById('user-affiliation').value = user.affiliation || 'none'; // ★所属を読み込む
...
function exportUsersToCSV() {
    ...
    const headers = ["利用者ID", "氏名", "呼び方", "所属事業所"];
    const affiliationMap = {
        'home': 'ホーム',
        'search': 'サーチ',
        'both': 'ホーム＆サーチ', // ★「両方」を「ホーム＆サーチ」に変更
    ...
    const rows = allUsers.map(user => [
        ...
        escapeCSV(affiliationMap[user.affiliation || 'none']) // ★所属を追加
    ]);
...
function exportRecordsToCSV() {
    ...
    const rows = sortedRecords.map(record => {
         ...
         const userAffiliation = user ? affiliationMap[user.affiliation || 'none'] : '不明';
         ...
         return [
            ...
            escapeCSV(userName),
            escapeCSV(userAffiliation), // ★所属を追加
